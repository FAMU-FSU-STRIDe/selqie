#include "legged_mpc/legged_mpc_2d.hpp"
...
static inline geometry_msgs::msg::Vector3 toVectorMsg(const OSQPVectorNC &v)
{
...
public:
  LeggedMPCNode() : Node("legged_mpc_node")
  {
    ...
    std::vector<double> force_weights = {1e-6, 1e-6};
    this->declare_parameter("force_weights", force_weights);
    this->get_parameter("force_weights", force_weights);
    _config.force_weights = Eigen::Map<OSQPVectorNC>(force_weights.data());
  ...
  void solve()
  {
    ...
    for (std::size_t i = 0, j = 0; i < _config.num_legs; i++)
    {
      ...
      {
        OSQPVectorNC ctrl = -sol.ustar[0].block<NUM_CONTROLS_PER_LEG, 1>(NUM_CONTROLS_PER_LEG * j++, 0);
