...
tmux new-session -d -s unitree-a1
...
tmux select-pane -t 0
tmux send-keys "source ${ROS2_WS}/install/setup.bash; ros2 launch unitree_a1_mujoco unitree_a1.launch.py" C-m
...
tmux send-keys "source ${ROS2_WS}/install/setup.bash; ros2 run robot_utils unitree_a1_util" C-m

# Now source the tmux configuration file to enable bindings and mouse support
tmux source-file $SCRIPT_DIR/.tmux.conf

# Attach to the session
tmux attach-session -t unitree-a1
